...
def ejemplito():
    ...
    FLUJO = solver.NumVar(0, solver.infinity(), 'FLUJO')
    print('Número de variables=', solver.NumVariables())

    # tenemos que hacer balance de nodos
    for n in nodos:

        # ahora tengo que modificar las restricciones
        if n == len(nodos):
            # estariamos en la ultima restriccion
            rnodos[n] = solver.Add(sum(A[n][a] * x[a] for a in arcos) == -FLUJO, 'RF%d' % (n))
        elif n == 1:
            # estariamos en la primera restriccion

            rnodos[n] = solver.Add(sum(A[n][a] * x[a] for a in arcos) == FLUJO, 'RF%d' % (n))
        else:
            # porque es un 0
            rnodos[n] = solver.Add(sum(A[n][a] * x[a] for a in arcos) == 0, 'RF%d' % (n))

    for a in arcos:
        solver.Add(x[a] <= info_arcos[a]['coste'])
